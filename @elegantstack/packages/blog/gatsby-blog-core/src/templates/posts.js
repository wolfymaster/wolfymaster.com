...
export const pageQuery = graphql`query PostsPageQuery($paginatePostsPage: Boolean!, $skip: Int, $limit: Int, $includeExcerpt: Boolean!, $includeTimeToRead: Boolean!, $imageQuality: Int!) {
  featuredPosts: allArticle(
    filter: {private: {ne: true}, draft: {ne: true}, featured: {eq: true}}
    sort: {date: DESC}
    limit: 10
  ) {
    nodes {
